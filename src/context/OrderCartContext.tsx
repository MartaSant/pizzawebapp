...
interface OrderCartContextValue {
  ...
  confirmOrder: (onSaved?: (snap: string) => void) => Promise<void>
...
export function OrderCartProvider({ children }: { children: ReactNode }) {
  ...
  const applyCartLoad = useCallback(
    (load: OrderCartLoad) => {
      setClientName(load.nomeCliente ?? '')
  ...
  const confirmOrder = useCallback(
    async (onSaved?: (snap: string) => void) => {
      try {
        ...
        resetCart()
        onSaved?.(order.receiptSnapshot)
        setMessage(null)
      } catch (e) {
        setMessage(e instanceof Error ? e.message : 'Errore')
      }
    },
    [user, clientName, pizzaLines, bibitaLines, resetCart],
  )
